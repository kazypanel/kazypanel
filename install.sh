...
# URL publique du panel (pour les mails)
SERVER_PUBLIC_IP_EARLY=$(curl -s --max-time 5 https://api.ipify.org 2>/dev/null || hostname -I | awk '{print $1}')
read -rp "  $(echo -e "${CYAN}URL publique du panel${NC} [http://${SERVER_PUBLIC_IP_EARLY}:${PANEL_PORT:-8080}] : ")" PANEL_PUBLIC_URL
PANEL_PUBLIC_URL=${PANEL_PUBLIC_URL:-"http://${SERVER_PUBLIC_IP_EARLY}:${PANEL_PORT:-8080}"}
...
echo -e "  • URL publique    : ${CYAN}${PANEL_PUBLIC_URL}${NC}"
...
# ── Fichier panel_config.json initial ────────────────────────
step "Initialisation de la configuration panel"
cat > "$INSTALL_DIR/panel_config.json" << PCFG
{
  "pmaUrl": "${PMA_URL}",
  "panelPublicUrl": "${PANEL_PUBLIC_URL}"
}
PCFG
chmod 600 "$INSTALL_DIR/panel_config.json"
ok "panel_config.json initialisé (URL : ${PANEL_PUBLIC_URL})"

# ── backup-cron.js — créer si absent du clone ─────────────────
if [ ! -f "$INSTALL_DIR/backup-cron.js" ]; then
  info "Création de backup-cron.js..."
  cat > "$INSTALL_DIR/backup-cron.js" << 'BCRON'
#!/usr/bin/env node
'use strict';
...
let retain = 7;
try { const cfg = JSON.parse(fs.readFileSync(CONFIG_FILE, 'utf8')); retain = cfg.backupSchedule?.retain || 7; } catch {}
if (!fs.existsSync(BACKUP_DIR)) fs.mkdirSync(BACKUP_DIR, { recursive: true });
const timestamp  = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
const backupName = `kazypanel-backup-${timestamp}.tar.gz`;
const backupPath = path.join(BACKUP_DIR, backupName);
try {
  execSync(`tar -czf "${backupPath}" --exclude="${INSTALL_DIR}/node_modules" --exclude="${INSTALL_DIR}/backups" --exclude="${INSTALL_DIR}/.git" -C "${path.dirname(INSTALL_DIR)}" "${path.basename(INSTALL_DIR)}"`, { timeout: 120000 });
  console.log(`[${new Date().toISOString()}] Sauvegarde créée : ${backupName}`);
} catch (err) { console.error(`[${new Date().toISOString()}] Erreur : ${err.message}`); process.exit(1); }
const files = fs.readdirSync(BACKUP_DIR).filter(f => f.startsWith('kazypanel-backup-') && f.endsWith('.tar.gz')).map(f => ({ name: f, mtime: fs.statSync(path.join(BACKUP_DIR, f)).mtime })).sort((a, b) => b.mtime - a.mtime);
if (files.length > retain) files.slice(retain).forEach(f => { try { fs.unlinkSync(path.join(BACKUP_DIR, f.name)); } catch {} });
BCRON
  chmod 755 "$INSTALL_DIR/backup-cron.js"
  ok "backup-cron.js créé"
else
  ok "backup-cron.js déjà présent"
fi
...
echo -e "  ┌─────────────────────────────────────────────────┐"
echo -e "  │  🔗 URL        : ${CYAN}${PANEL_PUBLIC_URL}${NC}"
